...
import { Plus, Package, X } from "@phosphor-icons/react";
...
function InboundModal({ open, onClose, skus, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <form
        onSubmit={submit}
        data-testid="inbound-modal"
        className="w-full max-w-lg bg-[#0A0A0A] border border-white/10 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]"
      >
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <select
              data-testid="inbound-sku"
              value={skuId}
              onChange={(e) => setSkuId(e.target.value)}
              className="w-full bg-transparent border border-white/20 focus:border-[#007AFF] outline-none px-3 py-2 text-white text-sm rounded-sm font-mono [&>option]:bg-[#0A0A0A]"
              required
            >
              {skus.map((s) => (
                <option key={s.id} value={s.id}>{s.master_sku_code} — {s.product_name}</option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                data-testid="inbound-qty" type="number" min="1" value={qty}
            ...
            <div>
              ...
              <input
                data-testid="inbound-base" type="number" step="0.01" min="0" value={baseCost}
            ...
            <div>
              ...
              <input
                data-testid="inbound-shipping" type="number" step="0.01" min="0" value={shipping}
          ...
          <div className="border border-[#007AFF]/40 bg-[#007AFF]/10 p-4">
            ...
            <div className="font-heading font-black text-3xl tracking-tighter mt-2 font-mono" data-testid="final-cogs">
              {fmt(finalCogs)}
            </div>
            <div className="text-[10px] font-mono uppercase tracking-widest text-zinc-500 mt-1">
              base + (shipping / quantity)
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-white/10 px-6 py-4 flex justify-end gap-3">
          <button type="button" onClick={onClose}
            className="border border-white/10 text-white hover:bg-white/5 px-4 py-2 rounded-sm text-xs uppercase tracking-wider transition-colors">
            Cancel
          </button>
          <button
  ...
  return (
    <div data-testid="inventory-page">
      <header className="border-b border-white/10 px-8 py-6 flex items-center justify-between">
        ...
        <button
          data-testid="open-inbound-modal"
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm" data-testid="inventory-table">
          ...
          <tbody>
            {loading && (
              <tr><td colSpan="6" className="p-8 text-center text-zinc-500 font-mono text-xs uppercase tracking-widest">Loading…</td></tr>
            )}
            {skus.map((s) => {
              const low = s.real_stock < 50;
